...
export const initRedis = async ({ redisReadUrl, redisWriteUrl }: InitRedisOptions) => {
  const logger = MonoContext.getStateValue('logger') as Logger;

  try {
    const readRedis = await createRedisConnection(redisReadUrl ?? REDIS_URL ?? '', true);
    const writeRedis = await createRedisConnection(redisWriteUrl ?? REDIS_WRITE_URL ?? '');
