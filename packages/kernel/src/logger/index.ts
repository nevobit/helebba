...
export type LogLevel = 'debug' | 'info' | 'warn' | 'error' | 'fatal' | 'all' | 'raw';
...
type TransportConstructor = new (options: LoggerTransportOptions) => LoggerTransport;
...
class ConsoleTransport implements LoggerTransport {
  ...
  async log(entry: LogEntry): Promise<void> {
    ...
    if (Object.keys(meta).length) {
      console.log(
        inspect({ ...information, ...meta }, { colors: true, depth: null, sorted: true }),
      );
    } else {
